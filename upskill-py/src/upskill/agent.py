...
from upskill.loop import run_agentic_loop, run_agentic_loop_structured
...
_active_agents: WeakSet[ChatAgent] = WeakSet()
...
class ChatAgent:
    ...
    def __init__(self, path: str | Path | None = None) -> None:
        ...
        """
        self._config: AgentConfig = load_agent(path)
        self._skill_manager: SkillManager = SkillManager.from_skills(self._config.skills)
    ...
    def run(self, messages: list[dict[str, Any]]) -> str:
        ...
        """
        loop = self._ensure_loop()
        future = asyncio.run_coroutine_threadsafe(self._run_async(messages), loop)
        return future.result()

    async def _run_async(self, messages: list[dict[str, Any]]) -> str:
...
class Agent(Generic[InputT, OutputT]):
    ...
    def run(self, input_value: InputT) -> OutputT:
        ...
        """
        loop = self._ensure_loop()
        future = asyncio.run_coroutine_threadsafe(self._run_async(input_value), loop)
        return future.result()

    async def _run_async(self, input_value: InputT) -> OutputT:
        """Async implementation of run()."""
        system_prompt = self._build_system_prompt()
    ...
    def _shutdown(self) -> None:
        """Internal shutdown - stops the event loop."""
        if self._closed:
            return
        self._closed = True
